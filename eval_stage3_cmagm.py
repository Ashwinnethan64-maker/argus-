...
def evaluate_model(config_dict, checkpoint_path, split_name, mode_name):
    print(f"\n" + "="*70)
    print(f"EVALUATING STAGE 3 CMAGM | MODE: {mode_name} | SPLIT: {split_name}")
    print("="*70)
    ...
    print(f"CMAGM Model File Size : {model_size_mb:.2f} MB")
    ...
    print("\n--- Running Official COCO Evaluation for Stage 3 CMAGM ---")
...
if __name__ == '__main__':
    results_file = "stage3_cmagm_results.json"
    ...
    for split in splits:
        for mode in modes:
            ...
            res = evaluate_model(cfg_dict, STAGE3_CHECKPOINT_PATH, split_name=split, mode_name=mode)
            all_benchmark_results[key] = res
            
            with open(results_file, "w") as f:
                json.dump(all_benchmark_results, f, indent=2)
            print(f"Saved Stage 3 progress to {results_file} after evaluating {key}")

    print("\nSaved full Stage 3 CMAGM benchmark results to stage3_cmagm_results.json")
