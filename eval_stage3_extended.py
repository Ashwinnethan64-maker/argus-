...
EXTENDED_WORK_DIR = os.path.abspath(os.path.join("work_dirs", "qfdet_cmagm_extended"))

def get_latest_checkpoint():
    """Find the latest checkpoint in extended work dir, fallback to stage3 or baseline."""
    latest_pth = os.path.join(EXTENDED_WORK_DIR, "latest.pth")
    if os.path.exists(latest_pth):
        return latest_pth
    
    epoch_pths = sorted(glob.glob(os.path.join(EXTENDED_WORK_DIR, "epoch_*.pth")))
    if epoch_pths:
        return epoch_pths[-1]
        
    stage3_latest = os.path.abspath(os.path.join("work_dirs", "qfdet_cmagm_stage3", "latest.pth"))
    if os.path.exists(stage3_latest):
        return stage3_latest
        
    return os.path.abspath(os.path.join("checkpoints", "qfdet_vtuav.pth"))

def evaluate_model(config_dict, checkpoint_path, split_name, mode_name):
    print(f"\n" + "="*70)
    print(f"EVALUATING CMAGM EXTENDED | CHECKPOINT: {os.path.basename(checkpoint_path)} | MODE: {mode_name} | SPLIT: {split_name}")
    print("="*70)
    ...
    print("\n--- Running Official COCO Evaluation for CMAGM Extended ---")
...
if __name__ == '__main__':
    results_file = "stage3_extended_results.json"
    ...
    print(f"Selected Checkpoint for Evaluation: {checkpoint_path}")
    ...
    for split in splits:
        for mode in modes:
            ...
            print(f"Saved extended evaluation progress to {results_file} after evaluating {key}")

    print(f"\nSaved full Stage 3 Extended CMAGM benchmark results to {results_file}")
